	...
	<div v-title="header.title">
		...
		<div class="feature" :style="{height: scrollHeight}">
			...
			<div class="feature-main">
				<div class="feature-item" v-for="(item, index) in featureData" :key="index">
					...
					<router-link :to="{path: 'book', query: {id: items.id}}" tag="div" class="feature-items" v-for="(items, index) in item.children" :key="index">
						...
						<div class="feature-items-main">
							...
							<p class="feature-items-category">
								...
								<span v-text="'人气'"></span>
								<span class="feature-items-major" v-text="items.majorCate"></span>
							</p>
						</div>
					</router-link>
				</div>
...
export default {
	...
	methods: {
		...
		getSwiperData () {
			this.$axios.get('/recommendPage/node/spread/'+this.id, {params: {
				pl: 'ios'
			}}).then((res)=>{
				for (let i=0; i<res.data.data.length; i++) {
		...
		getFeatureData () {
			for (let i=0; i<this.featureData.length; i++) {
				this.$axios.get('/recommendPage/books/'+this.featureData[i].id, {}).then((res)=>{
					for (let j=0; j<res.data.data.length; j++) {
						this.featureData[i].children.push({
							...
							latelyFollower: this.formatPeople(res.data.data[j].book.latelyFollower)
						})
					}
				})
			}
