<template>
	<div v-title="header.title">
		...
		<div class="category-filter">
			...
			<div class="category-filter-item">
				<cube-checker v-model="checkerClassifyChild" :options="checkerClassifyChildOptions" type="radio"/>
			</div>
		</div>
...
export default {
	...
	computed: {
		scrollHeight () {
			return (document.body.clientHeight - 34.13333*document.body.clientWidth/100)+'px'
		},
		options () {
			return {
	      pullUpLoad: {
	...
	methods: {
		getData (type) {
			this.$axios.get("/book/by-categories",{params: {
					gender: this.$route.query.gender,
					type: this.checkerType,
					major: this.$route.query.name,
					minor: this.checkerClassifyChild === 'all' ? '' : this.checkerClassifyChild,
		...
		getClassifyChild () {
			this.$axios.get("/cats/lv2",{}).then(res=>{
				for (let i=0; i<res.data[this.$route.query.gender].length; i++) {
					if (res.data[this.$route.query.gender][i].major === this.$route.query.name) {
						for (let j=0; j<res.data[this.$route.query.gender][i].mins.length; j++) {
