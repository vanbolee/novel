<template>
	<cube-popup type="read-menu" position="left" @mask-click.stop="maskClick" ref="menu">
	  <div class="read-menu" :style="{transform: inshow ? 'translateX(0%)' : 'translateX(-100%)'}" @touchmove.stop @click.stop>
	  	<div class="read-menu-main">
	  		<div class="read-menu-main-title">{{this.$parent.bookName}}</div>
	  		<div class="read-menu-main-text">目录</div>
	  		<div class="read-menu-main-item" v-for="(item, index) in data" :key="index" @click="changeChapterIndex(index)">
	  			<p :style="{color: $parent.source.menuIndex-1 === index ? '#8ECDFC' : ''}">{{item.title}}</p>
	  		</div>
	  	</div>
	  </div>
	</cube-popup>
